...
# -----------------------------------------------------------------------------
# combat_influence
# Called by GuardAgent each physics frame during combat.
# Returns a fill amount for CombatMeterComponent based on current visual state.
# GuardAgent passes this straight to combat_meter.add_to_meter(amount * delta).
# -----------------------------------------------------------------------------
func combat_influence(delta: float) -> float:
	if not _detection_confirmed:
		return 0.0

	var amount: float = 1.0  # base: confirmed sighting

	if _last_seen_body != null:
		var dist = body.global_position.distance_to(_last_seen_body.global_position)
		if dist <= strike_distance:
			amount = 4.0   # gap closed — full pressure
		elif dist <= danger_distance:
			amount = 2.5   # danger range — elevated
		elif dist <= alert_distance:
			amount = 1.0   # alert range — baseline
